...
export function AuthSheet({ open, onOpenChange }: AuthSheetProps) {
  ...
  async function handleSignIn(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (err) { setError(translateError(err)); setIsLoading(false) }
    else { setIsLoading(false); onOpenChange(false) }
  }
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="bottom"
        className="rounded-t-2xl px-0 pb-0 max-h-[90dvh] flex flex-col z-[60]"
        style={{ background: "var(--bg)", borderTop: "2px solid var(--border-hard)" }}
      >
        <SheetHeader
          className="px-6 pb-4 border-b border-ink/10"
        ...
        <div className="overflow-y-auto flex-1">
          <Tabs value={tab} onValueChange={(v) => { setTab(v as "login" | "signup"); resetError() }}>
            <TabsList className="mx-6 mb-4 mt-4 w-[calc(100%-3rem)] border-2 border-ink" style={{ background: "var(--paper-2)" }}>
              <TabsTrigger value="login" className="flex-1 data-[state=active]:bg-ink data-[state=active]:text-[var(--bg)]">Connexion</TabsTrigger>
              <TabsTrigger value="signup" className="flex-1 data-[state=active]:bg-ink data-[state=active]:text-[var(--bg)]">Inscription</TabsTrigger>
            </TabsList>
          ...
          <div className="px-6 pb-[calc(1.5rem+env(safe-area-inset-bottom,20px))] pt-4">
            <div className="flex items-center gap-3 py-2">
              <Separator className="flex-1 bg-ink/20" />
            ...
            <div className="flex flex-col gap-2 pt-2">
              <Button
                variant="outline"
                className="w-full gap-2 border-2 border-ink bg-[var(--bg)] text-ink hover:bg-paper-2"
                disabled={isLoading}
